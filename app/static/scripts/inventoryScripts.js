...
function load_iventory(inventory_products) {
  ...
  for (product of inventory_products) {
    ...
    function open_card() {
      ...
      // check global for open card, close if required, modify array
      if(current_open_card.length > 0) {
        let opened_card = current_open_card.pop();
        console.log(opened_card);
        let close_button = opened_card.firstChild;
        close_button.click();

        current_open_card.push(this);
      } else {
        current_open_card.push(this);
      }
    ...
    for (let photo_src of product.photos_list) {
      ...
      lr_inv_product_photo.src =
        "/static/product_inventory/" + product.id + "/LowRes/" + photo_src;
      ...
      hr_inv_product_photo.src =
        "/static/product_inventory/" + product.id + "/HighRes/" + photo_src;
...
function toggle_selected_card(card_element) {
  ...
  let close_selected_product = document.getElementsByClassName(
    "close_selected_product"
  );
